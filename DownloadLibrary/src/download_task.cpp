...
DownloadLibrary::DownloadTaskMultiple::DownloadTaskMultiple(std::string task_location,sharedPoolType connectionPool ,std::string user_agent, std::string dns, bool factory_flag):connectionPool(connectionPool),DownloadLibrary::DownloadTask(task_location, user_agent,dns){
    ...
    for( auto n : cfg_data["parts"]){
        ...
        this->parts.push_back({
            std::make_shared<DownloadLibrary::CurlRequest>(this->connectionPool , this->url, this->task_dir / n["name"],false,buff,this->user_agent,dns),
            id,
...
part_count(part_count),parts(part_count),follow_redirects(follow_redirects),header(header),connectionPool(connectionPool){
    ...
    for(int i=0;i<part_count;i++)
    {
        ...
        parts[i]={
            std::make_shared<DownloadLibrary::CurlRequest>(connectionPool, this->url,task_dir / gen_part_name(i),false,buff,this->user_agent,dns),
            i,
            0
        };
...
DownloadLibrary::ReqsType DownloadLibrary::DownloadTaskMultiple::get_requests(){
 ...
 std::string DownloadLibrary::DownloadTask::getFinalFileName(){
     return this->final_props.file_name;
 }
